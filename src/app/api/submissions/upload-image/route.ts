...
export async function POST(request: Request) {
  ...
  if (meta.ip) {
    ...
    const recentCount = await prisma.auditLog.count({
      where: { action: "submission_upload_image", createdAt: { gte: since }, ip: meta.ip },
    });
    if (recentCount >= 8) return NextResponse.json({ error: "rate_limited" }, { status: 429 });
  }
  ...
  if (buf.length === 0 || buf.length > 1_500_000) return NextResponse.json({ error: "too_large" }, { status: 413 });
  ...
  if (!(size.width === 1280 && size.height === 720)) {
    return NextResponse.json({ error: "wrong_dimensions" }, { status: 400 });
  }
  ...
  await logAudit({
    action: "submission_upload_image",
    entityType: "File",
    entityId: fileName,
    metadata: { publicPath, bytes: buf.length, width: size.width, height: size.height, mime, ip: meta.ip, userAgent: meta.userAgent },
  });
