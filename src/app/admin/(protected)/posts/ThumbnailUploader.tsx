...
async function cropToDataUrl(imageSrc: string, area: Area) {
...
export function ThumbnailUploader({
  ...
  const onConfirm = useCallback(async () => {
    ...
    setStatus("uploading");
    const dataUrl = await cropToDataUrl(imageSrc, croppedAreaPixels).catch(() => null);
    if (!dataUrl) {
      setStatus("error");
      return;
    }
  ...
  return (
    <div className="grid gap-3">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="text-xs tracking-[0.22em] text-white/45">縮圖</div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-h-[calc(100dvh-2rem)] max-w-3xl overflow-y-auto rounded-2xl border border-white/10 bg-zinc-950/90 backdrop-blur">
            <div className="flex items-center justify-between border-b border-white/10 px-5 py-4">
              <div className="text-sm font-semibold tracking-wide text-white/85">裁切縮圖</div>
